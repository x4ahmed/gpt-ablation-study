...
REM Muon dropout low ablation: dropout 0.0
...
echo.
echo Running Muon low dropout experiment...
echo.
python muon_runs/train.py ^
  --run-name muon_dropout_low ^
  --wandb_entity i-learn ^
  --no_torch_compile ^
  --n_layer 4 --n_embd 512 --n_head 8 ^
  --device-batch-size 2 --num-epochs 16 ^
  --lr_multiplier 0.8 --weight-decay 0.8 --dropout 0.0 ^

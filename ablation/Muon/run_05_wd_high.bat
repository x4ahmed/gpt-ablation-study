...
REM Muon WD high ablation: weight_decay 1.2 (scaled WD schedule)
...
echo.
echo Running Muon high weight-decay experiment...
echo.
python muon_runs/train.py ^
  --run-name muon_wd_high ^
  --wandb_entity i-learn ^
  --no_torch_compile ^
  --n_layer 4 --n_embd 512 --n_head 8 ^
  --device-batch-size 2 --num-epochs 16 ^
  --lr_multiplier 0.8 --weight-decay 1.2 --wd-mid 0.15 --wd-end 1.875 --dropout 0.1 ^

...
parser.add_argument("--no_torch_compile", action="store_true",
                    help="Disable torch.compile to avoid Windows/Inductor compiler issues")
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    @torch.no_grad()
    def step(self):
        # Prefer the distributed default group if initialized; otherwise fall back
        # to the local get_dist_info() helper which reads env vars or defaults.
        if dist.is_available() and dist.is_initialized():
            rank, world_size = dist.get_rank(), dist.get_world_size()
        else:
            _, rank, _, world_size = get_dist_info()

        # Single-process fallback: use a local AdamW optimizer to update params
        # instead of calling distributed collectives which require an initialized
        # process group.
        if world_size <= 1:
            # Build local param groups compatible with torch.optim.AdamW
            local_groups = []
            for group in self.param_groups:
                lg = {k: v for k, v in group.items() if k in ("params", "lr", "weight_decay", "betas", "eps")}
                # Ensure required keys exist
                if "betas" not in lg: lg["betas"] = ADAM_BETAS
                if "eps" not in lg: lg["eps"] = 1e-8
                local_groups.append(lg)
            local_opt = torch.optim.AdamW(local_groups)
            local_opt.step()
            return
        reduce_infos = []
        for group in self.param_groups:
...
if device_type == "cuda":
    ...
    elif "3050" in gpu_name or "3060" in gpu_name or "3070" in gpu_name or "3080" in gpu_name or "3090" in gpu_name:
        # Ampere consumer GPUs: use device properties for a reasonable estimate
        props = torch.cuda.get_device_properties(device)
        clock_rate = getattr(props, "clock_rate", 0)
        if clock_rate:
            gpu_peak_flops = props.multi_processor_count * 128 * 2 * clock_rate * 1e3
        else:
            # Fallback: ~1.5 GHz boost clock for Ampere mobile/desktop
            gpu_peak_flops = props.multi_processor_count * 128 * 2 * 1.5e9
        print0(f"Using Ampere GPU peak FLOPs for {gpu_name}: {gpu_peak_flops:.1e}")
    else:
        props = torch.cuda.get_device_properties(device)
        clock_rate = getattr(props, "clock_rate", 0)
        if clock_rate:
            ...
            # Fallback: assume 1.5 GHz if clock_rate is unavailable
            gpu_peak_flops = props.multi_processor_count * 128 * 2 * 1.5e9
...
if args.no_torch_compile:
    print0("Skipping torch.compile due to Windows/Inductor compiler limitations")
else:

...
\begin{frame}{Raw Validation Loss}
  \small
  At the end of epoch $e$, the current raw model weights are denoted by $\theta_e$;
  $E=16$ denotes the final epoch.
  ...
  \]
  The model is penalized when it assigns low probability to the observed next token $t_i$.
  ...
  \]
  Padding contributes neither loss nor token count; lower validation loss is better.

  \vspace{0.35cm}
  \begin{block}{Raw candidates}
    The values $\mathcal{L}_{\mathrm{val}}(\theta_e)$ for epochs $e=1,\ldots,E$
    are the raw candidates used on the next slide.
  \end{block}
\end{frame}

\begin{frame}{Best Validation Loss}
  \small
  \textbf{1. Final EMA candidate}
  \[
    \bar\theta_s=\beta\bar\theta_{s-1}+(1-\beta)\theta_s,
    \qquad s=1,\ldots,S
  \]
  Here $s$ indexes optimizer steps and $S$ is the final training step; evaluate
  $\mathcal{L}_{\mathrm{val}}(\bar\theta_S)$ once after training.

  \vspace{0.3cm}
  \textbf{2. Last-four checkpoint-average candidate}
  \[
    \theta_{\mathrm{avg}}
      =\sum_{j=1}^{4}\frac{j}{10}\,\theta_{E-4+j}
      =\frac{1\theta_{E-3}+2\theta_{E-2}+3\theta_{E-1}+4\theta_E}{10}
  \]
  Thus the most recent raw checkpoint receives the largest weight.

  \vspace{0.3cm}
  \textbf{3. Reported best validation loss}
  \[
    ...
    \right\}
  \]
  Report the minimum value together with its source: raw epoch, EMA, or checkpoint average.
\end{frame}
...
\begin{frame}{Hybrid Optimizer}
  \small
  \begin{tabular}{p{0.29\textwidth}p{0.18\textwidth}p{0.18\textwidth}p{0.19\textwidth}}
    \toprule
